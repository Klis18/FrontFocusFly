...
export class WeekTasksComponent implements OnInit, OnDestroy {
  ...
  getTasksCount(state: string) {
    this.countSubscription = this.tasksService.getWeekTasks(state).subscribe(
      res => {
        this.pendingTasksCount    = (state == 'Pendiente')  ? res.length : 0;
        this.inProgressTasksCount = (state == 'En Proceso') ? res.length : 0;
        this.finishedTasksCount   = (state = 'Finalizado')  ? res.length : 0;
      }
    );
  }
